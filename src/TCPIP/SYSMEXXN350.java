 ...
 * @author GHSS-BLIS
 */
public class SYSMEXXN350 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("SYSMEX XN-350 handler stopped", DisplayMessageType.TITLE);
      ...
      @Override
    public void run() {
        log.AddToDisplay.Display("SYSMEX XN-350 handler started...", DisplayMessageType.TITLE);
        ...
        try
        {
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
                ...
                log.AddToDisplay.Display("SYSMEX XN-350 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                ClientThread client = new ClientThread(connSock,"SYSMEX XN-350");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("SYSMEX XN-350 client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
         ...
         try
         {
            ...
            {            
                ...
                {               
                       ...
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to SYSMEX XN-350",DisplayMessageType.INFORMATION);
        ...
        try
        {
            ...
            else if (type == MSGTYPE.RESULTS)
            {
                ...
                if(pidParts.length > 5)
                {
                    ...
                    for(int i=5;i<=48;i++)
                    {
                        if(!msgParts[i].split("\\|")[0].startsWith("R"))
                        {
                            break;
                        }
                        String tmp = msgParts[i].split("\\|")[2].split("\\^")[4];
                            mID = getMeasureID(tmp);
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/SYSMEX/SYSMEXXN350.xml");
        try {
            data = p.getMicros60Filter(whichdata);           
        } catch (Exception ex) {
            Logger.getLogger(SYSMEXXN350.class.getName()).log(Level.SEVERE, null, ex);
        }        
        return data;        
    }
